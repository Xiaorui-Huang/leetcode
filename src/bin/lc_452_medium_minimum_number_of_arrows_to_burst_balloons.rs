...
#[rstest]
#[case([[10,16],[2,8],[1,6],[7,12]].iter().map(|x|x.to_vec()).collect(), 2)]
#[case([[1,2],[3,4],[5,6],[7,8]].iter().map(|x|x.to_vec()).collect(), 4)]
#[case([[1,2],[2,3],[3,4],[4,5]].iter().map(|x|x.to_vec()).collect(), 2)]
fn test_find_min_arrow_shots(#[case] points: Vec<Vec<i32>>, #[case] expected: i32) {
    assert_eq!(Solution::find_min_arrow_shots(points), expected);
}
