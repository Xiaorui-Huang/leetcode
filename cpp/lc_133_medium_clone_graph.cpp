...
class Solution {
  public:
    ...
    Node *get_node_clone(Node *node) {
        auto node_it = nodes_map.find(node->val);
        auto *node_clone = (node_it != nodes_map.end()) ? node_it->second
                                                        : new Node(node->val);
        return node_clone;
    }
    Node *cloneGraph(Node *node) {
        ...
        while (!q.empty()) {
            auto node = q.front();
            q.pop();
            auto *node_clone = get_node_clone(node);

            for (auto &neighbor : node->neighbors) {
                if (!nodes_map.count(neighbor->val))
                    q.push(neighbor);

                auto neighbor_clone = get_node_clone(neighbor);
                node_clone->neighbors.push_back(neighbor_clone);
            }
        }

        return nodes_map.at(1);
    }
};
// @lc code=end
