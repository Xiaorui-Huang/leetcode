...
# @lc code=start
class Solution:
    def findMinArrowShots(self, points: list[tuple[int, int]]) -> int:
        by_end_point: Callable[[tuple[int, int]], int] = lambda point: point[1]
        points.sort(key=by_end_point)

        _, arrow_pos = points[0]
        count = 1
