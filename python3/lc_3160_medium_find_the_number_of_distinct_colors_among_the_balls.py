...
class Solution:
    def queryResults(self, limit: int, queries: list[list[int]]) -> list[int]:
        ...
        ball_map = {}  # not in map means 0 and in map means exitst with some color

        result = [0] * n

        for i, (ball, color) in enumerate(queries):
            if ball in ball_map:
                cur_color = ball_map[ball]
                counter[cur_color] -= 1
                if counter[cur_color] == 0:
                    del counter[cur_color]

            counter[color] += 1
            ball_map[ball] = color

            result[i] = len(counter)
        return result
