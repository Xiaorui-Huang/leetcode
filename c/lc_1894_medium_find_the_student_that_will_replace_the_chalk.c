/*
 * @lc app=leetcode id=1894 lang=c
 *
 * [1894] Find the Student that Will Replace the Chalk
 *
 * https://leetcode.com/problems/find-the-student-that-will-replace-the-chalk/description/
 *
 * algorithms
 * Medium (45.08%)
 * Likes:    1121
 * Dislikes: 128
 * Total Accepted:    173.1K
 * Total Submissions: 324.6K
 * Testcase Example:  '[5,1,5]\n22'
 *
 * There are n students in a class numbered from 0 to n - 1. The teacher will
 * give each student a problem starting with the student number 0, then the
 * student number 1, and so on until the teacher reaches the student number n -
 * 1. After that, the teacher will restart the process, starting with the
 * student number 0 again.
 * 
 * You are given a 0-indexed integer array chalk and an integer k. There are
 * initially k pieces of chalk. When the student number i is given a problem to
 * solve, they will use chalk[i] pieces of chalk to solve that problem.
 * However, if the current number of chalk pieces is strictly less than
 * chalk[i], then the student number i will be asked to replace the chalk.
 * 
 * Return the index of the student that will replace the chalk pieces.
 * 
 * 
 * Example 1:
 * 
 * 
 * Input: chalk = [5,1,5], k = 22
 * Output: 0
 * Explanation: The students go in turns as follows:
 * - Student number 0 uses 5 chalk, so k = 17.
 * - Student number 1 uses 1 chalk, so k = 16.
 * - Student number 2 uses 5 chalk, so k = 11.
 * - Student number 0 uses 5 chalk, so k = 6.
 * - Student number 1 uses 1 chalk, so k = 5.
 * - Student number 2 uses 5 chalk, so k = 0.
 * Student number 0 does not have enough chalk, so they will have to replace
 * it.
 * 
 * Example 2:
 * 
 * 
 * Input: chalk = [3,4,1,2], k = 25
 * Output: 1
 * Explanation: The students go in turns as follows:
 * - Student number 0 uses 3 chalk so k = 22.
 * - Student number 1 uses 4 chalk so k = 18.
 * - Student number 2 uses 1 chalk so k = 17.
 * - Student number 3 uses 2 chalk so k = 15.
 * - Student number 0 uses 3 chalk so k = 12.
 * - Student number 1 uses 4 chalk so k = 8.
 * - Student number 2 uses 1 chalk so k = 7.
 * - Student number 3 uses 2 chalk so k = 5.
 * - Student number 0 uses 3 chalk so k = 2.
 * Student number 1 does not have enough chalk, so they will have to replace
 * it.
 * 
 * 
 * 
 * Constraints:
 * 
 * 
 * chalk.length == n
 * 1 <= n <= 10^5
 * 1 <= chalk[i] <= 10^5
 * 1 <= k <= 10^9
 * 
 * 
 */

#include <stdlib.h> 
// @lc code=start

int binary_search_right(unsigned long arr[], unsigned long target, int low, int high){
    int mid, num, upper = high;
    while(low <= high){
        mid = low + (high - low) / 2;
        num = arr[mid];

        if(target <= num){
           high = mid - 1; 
        }else{//return the else part after search
           low = mid + 1;
        }
    }
    // suppose target is in arr
    // since by the end low > high
    // and when target == num -> high = mid - 1
    // this implies arr[high] < target
    // so we pick the low, which is one above high -> arr[low] == target 
    return low; 
    // if target is not in arr... then this work as intended, and could return (highest index + 1 aka not found)
    // i.e. target >= all num in arr -> low > high, where high never changed (if high is the highest index then low if out of bound)
}

int chalkReplacer(int* chalk, int chalkSize, int k) {
    unsigned long * prefix_sum = (unsigned long* )malloc(sizeof(unsigned long) * chalkSize);
    prefix_sum[0] = chalk[0];
    for (int i = 1; i < chalkSize; i++)
        prefix_sum[i] = prefix_sum[i - 1] + chalk[i];        
    
    unsigned long target = k % prefix_sum[chalkSize - 1];

    // binary search for the idx of the smallest number that is >= the remainder
    unsigned long candidate_idx = binary_search_right(prefix_sum, target, 0, chalkSize - 1);
    if (target == prefix_sum[candidate_idx])
        candidate_idx++;
    return candidate_idx % chalkSize; // in case +1 wraps around
}
// @lc code=end

